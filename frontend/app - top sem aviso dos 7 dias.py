...
def tela_mark():
    ...
    # 🔹 Histórico do chat (estilo ChatGPT: do mais antigo para o mais recente)
    for autor, mensagem in st.session_state.chat:
        st.markdown(f"**{autor}**: {mensagem}")

    st.markdown("---")

    # 🔹 Linha de input: texto + (futuro) anexos + (futuro) áudio + botão enviar
    col_input, col_file, col_audio, col_btn = st.columns([5, 1, 1, 1])

    with col_input:
        pergunta = st.text_input("📩 Sua pergunta:", key="input_mark")

    # Só interface por enquanto – ainda não mandamos arquivo/voz pro backend
    with col_file:
        st.file_uploader("📎", label_visibility="collapsed", key="upload_mark")
        st.caption("📎")

    with col_audio:
        st.caption("🎙️ Voz (em breve)")

    enviar = col_btn.button("Enviar", key="btn_enviar_mark")
    ...
    if enviar and pergunta:
        ...
        try:
            ...
            # terminou o streaming: grava no histórico e limpa o status
